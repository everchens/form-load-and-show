...
<?php
// 定义变量并设置为空值
$start_dateErr = $stop_dateErr = $nameErr = $departmentErr = $contractErr = $positionErr = $verifyErr = "";
$d1 = $d2 = $start_date = $stop_date = $name = $department = $contract = $position = $verify = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   if (empty($_POST["stop_date"])) {
	...
	if (!empty($_POST["verify"])) {
		...
		// 检查部门是否全是汉字
		if (!preg_match("/^[\x{4e00}-\x{9fa5}]+$/u",$verify)) {
			$verifyErr ="只允许汉字"; 
		}
	}
}
...
<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>"> 
   ...
   <br><br>
   人证验证：<input type="text" name="verify" value="<?php echo $verify;?>">
   <span class="error"> <?php echo $verifyErr;?></span>
   <br><br>
...
<?php
   ...
   if(!$db){
	...
	if(!empty($verify))
	{
		$condition7 = "人证验证 LIKE '%{$verify}%'";
	}
	
	$sql = "select COUNT(*) from personnel WHERE 
	...
	{$condition1} AND {$condition2} AND {$condition3} AND {$condition4} AND {$condition5} AND {$condition6}  AND {$condition7} ORDER BY 单位, 返岗日期, 轨迹";
